// Implement A-Star Search in C: f(x) = g(x) + h(x)
#include <stdio.h>
#define MAX_NODES 10
#define INFINITY 9999
typedef struct {
    int heu;						// Heuristic Value
	int adj[MAX_NODES - 1];			// Adjascent Vertices List
	int cost[MAX_NODES - 1];		// Cost to reach Adjascent Vertex
} Node;
...
{
	for (int i = 0; i < MAX_NODES - 1; i++) {							// Adjuscent nodes can have max N-1 nodes 
...
int aStar(Node graph[], int start, int goal) {
    ...
    while (current != goal) {
		...
		// Go to unvisited adjascent node with smallest heiuristic value
...
int main() {
    ...
    for (int i = 0; i < MAX_NODES; i++) {
		for (int j = 0; j < MAX_NODES - 1; j++) {			// Adjuscent nodes can have max N-1 nodes 
			graph[i].adj[j] = -1;
		}
    }
	// Set Adjuscent vertices list for individual nodes with cost(Make graph in paper)
